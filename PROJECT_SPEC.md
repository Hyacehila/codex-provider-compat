...
### 3.8 2026-07-11 上游复核快照

以下事实检查于 2026-07-11 15:20:49 UTC（北京时间 23:20:49）。这是带时间的上游快照，不是对未来 `main` 的永久声明；最终交付前必须再次刷新：

- 当时最新正式 release 为 `rust-v0.144.1`（2026-07-09 发布）；检查的 main 提交为 `54b8f112a3815ead40ebcd50f9c2f2fc786e26fb`。
...
用户应能在下载当前源码仓库（或未来正式 Release）后，通过一条本地命令完成：
...
- 捕获到普通异常、SIGINT 或 SIGTERM 时立即按日志恢复，不能先清锁再继续执行。
...
报告必须区分：本地自动测试、CI 平台测试和真实 provider 人工测试，不能把未执行的测试写成已通过。
...
- 日志和状态文件不包含秘密。
- 如果创建 Release，必须提供文件 SHA-256；首次公开源码推送不创建 tag 或 Release，因此 README 不得假装已有 Release 下载物。
- 明确标记非官方社区工具。
...
6. Windows 快速使用。
7. macOS 快速使用。
8. `doctor/apply/status/rollback` 说明。
9. 工具实际会修改哪些文件。
10. 工具明确不会做什么。
11. 已知风险和后果。
12. Codex 更新后的处理。
...
快速安装不要只提供不透明的 `curl | sh` 或 `irm | iex`。首次公开源码交付的主要路径应是从 <https://github.com/Hyacehila/codex-provider-compat> 选择经 Actions 验证的固定 commit，下载该 commit 的 ZIP 或 clone 后 checkout 到它，校验脚本 SHA-256、查看脚本后本地运行。只有未来真正发布带稳定产物和校验和的 GitHub Release 后，README 才可把 Release 改为主要安装入口。可以额外给出一键方式，但必须标明信任含义。
...
- 中英文 README。
- 完整自动测试。
- 首次公开交付先提供经 Actions 验证的源码仓库，不创建 tag 或 GitHub Release。
- 后续正式 Release 必须提供 ZIP、独立脚本和 SHA-256，并在受控人工验证后再发布。
...
15. 只在所有自动测试通过后完成首次公开源码交付；未来准备 v0.1.0 Release 时还必须完成受控人工验证并发布独立 SHA-256。
