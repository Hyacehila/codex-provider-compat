...
[English](README.md) · [下载 v0.2.1](https://github.com/Hyacehila/codex-provider-compat/releases/tag/v0.2.1) · [自动测试](https://github.com/Hyacehila/codex-provider-compat/actions/workflows/test.yml)
...
打开 [v0.2.1 Release 页面](https://github.com/Hyacehila/codex-provider-compat/releases/tag/v0.2.1)，下载 `SHA256SUMS.txt` 和对应平台的压缩包：
...
如果你在 v0.1.1 中使用过现已移除的 `--enable-web-search`，并希望工具不再拥有该旧修改，请使用 v0.2.0 或更高版本运行 `rollback`，重启 Codex，再执行普通 `apply` 并重新启动到新任务。新版本不会改变你的搜索设置。
...
独立 CLI、Codex Desktop 和 app-server 组件可能分别更新，但共享同一个 Codex home 与模型目录。检测到不同版本时，工具不会自动替你选择。只有明确知道应用补丁后将完全重启并使用哪个 CLI 或 Desktop surface，才应传入对应的 `--codex-version`。
