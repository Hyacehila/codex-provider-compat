...
Test-Case 'apply/status/rollback full cycle, semantic catalog protection, and secret-free state' {
    ...
    Assert-Equal 'responses-lite-standard-tools' $state.patch_id 'patch id';Assert-Equal '0.2.1' $state.patch_version 'patch version';Assert-True ($state.config.PSObject.Properties.Name -contains 'previous_model_catalog_json_literal') 'state stores minimal literal';Assert-True ($state.config.PSObject.Properties.Name -contains 'original_mode') 'cross-platform state field original_mode missing';Assert-Equal $null $state.config.original_mode 'Windows original_mode should be null';Assert-Equal $false $state.config.web_search_modified 'new apply must not own web_search';Assert-Equal $false $state.config.previous_web_search_present 'new apply must not record a previous web_search value';Assert-Equal $null $state.config.previous_web_search 'new apply stored a previous web_search value';Assert-Equal $null $state.config.previous_web_search_literal 'new apply stored a previous web_search literal';Assert-False ($state.config.PSObject.Properties.Name -contains 'previous_model_catalog_json_line') 'state must not store full config lines';Assert-NotContains $stateText '# user choice' 'state leaked a config comment';Assert-NotContains $stateText 'provider.example.invalid' 'state leaked provider configuration'
...
Test-Case 'new apply preserves arbitrary web_search text and legacy v0.1.1 state remains rollback-compatible' {
    $basePrefix="model = `"gpt-5.6-sol`"`nmodel_provider = `"custom`"`n"
    $baseSuffix="`n[model_providers.custom]`nwire_api = `"responses`"`n"
    $forms=[ordered]@{
        absent=''
        live='web_search = "live" # preserve-live'
        cached='web_search = "cached" # preserve-cached'
        disabled='web_search = "disabled" # preserve-disabled'
        complex='web_search = { mode = "live", note = "preserve-complex" }'
        dotted='web_search.mode = "live" # preserve-dotted'
        duplicate="web_search = `"cached`" # preserve-first`nweb_search = `"disabled`" # preserve-second"
        same_line_multiline='web_search = """live""" # preserve-same-line-multiline'
        multiline_basic="web_search = `"`"`"`n  preserve-multiline-basic`n`"`"`""
        multiline_array="'web_search' = [`n  `"live`",`n  `"cached`",`n] # preserve-multiline-array"
    }
    foreach($entry in $forms.GetEnumerator()){
        $root=New-TestHome ("web-"+$entry.Key);$config=Join-Path $root 'config.toml';$middle=if($entry.Value){$entry.Value+"`n"}else{''};Write-Utf8 $config ($basePrefix+$middle+$baseSuffix);$before=(Get-FileHash $config).Hash
        $apply=Invoke-Tool (Apply-Args $root);Assert-Equal 0 $apply.ExitCode "$($entry.Key) apply: $($apply.Output)";Assert-NotContains $apply.Output 'plan: set web_search' "$($entry.Key) advertised a web_search mutation"
        $state=Read-StateJson $root;Assert-Equal $false $state.config.web_search_modified "$($entry.Key) state owned web_search";Assert-Equal $false $state.config.previous_web_search_present "$($entry.Key) state recorded web_search";Assert-Equal $null $state.config.previous_web_search "$($entry.Key) state leaked web_search";Assert-Equal $null $state.config.previous_web_search_literal "$($entry.Key) state leaked web_search literal";Assert-Equal 0 (Invoke-Tool (Status-Args $root)).ExitCode "$($entry.Key) status"
        $applied=[IO.File]::ReadAllText($config,[Text.Encoding]::UTF8);if($entry.Value){Assert-Contains $applied ([string]$entry.Value) "$($entry.Key) web_search text changed"}else{Assert-NotContains $applied 'web_search' 'absent web_search was inserted'}
        Assert-Equal 0 (Invoke-Tool (Rollback-Args $root)).ExitCode "$($entry.Key) rollback";Assert-Equal $before (Get-FileHash $config).Hash "$($entry.Key) config was not restored exactly"
    }

    $legacy=New-TestHome 'legacy-web';$legacyConfig=Join-Path $legacy 'config.toml';$legacyOriginal=$basePrefix.Replace("`n","`r`n")+"'web_search' = 'disabled' # legacy-comment`r`n"+$baseSuffix.Replace("`n","`r`n");Write-Utf8 $legacyConfig $legacyOriginal $true;Set-TestPrivateAcl $legacyConfig;$legacyHash=(Get-FileHash $legacyConfig).Hash;$legacyAcl=(Get-Acl $legacyConfig).Sddl;Assert-Equal 0 (Invoke-Tool (Apply-Args $legacy)).ExitCode 'legacy setup apply'
    $legacyText=[IO.File]::ReadAllText($legacyConfig,[Text.Encoding]::UTF8).Replace("'web_search' = 'disabled' # legacy-comment","'web_search' = `"live`" # legacy-comment");Write-Utf8 $legacyConfig $legacyText $true;$legacyState=Read-StateJson $legacy;$legacyState.patch_version='0.1.1';$legacyState.config.web_search_modified=$true;$legacyState.config.previous_web_search_present=$true;$legacyState.config.previous_web_search='disabled';$legacyState.config.previous_web_search_literal="'disabled'";Write-Json (Join-Path $legacy 'provider-compat-state.json') $legacyState;$legacyAppliedHash=(Get-FileHash $legacyConfig).Hash
    Assert-Equal 0 (Invoke-Tool (Status-Args $legacy)).ExitCode 'legacy state status';$legacyRecovery=Invoke-Tool (Rollback-Args $legacy) @{CODEX_PROVIDER_COMPAT_TEST_FAIL_STAGE='rollback-after-config'};Assert-Equal 3 $legacyRecovery.ExitCode 'legacy rollback fault was not surfaced';Assert-Equal $legacyAppliedHash (Get-FileHash $legacyConfig).Hash 'legacy rollback recovery did not restore the applied config';Assert-Equal 0 (Invoke-Tool (Status-Args $legacy)).ExitCode 'legacy state unhealthy after rollback recovery';Assert-Equal 0 (Invoke-Tool (Rollback-Args $legacy)).ExitCode 'legacy state rollback';Assert-Equal $legacyHash (Get-FileHash $legacyConfig).Hash 'legacy rollback did not restore exact config bytes';Assert-Equal $legacyAcl (Get-Acl $legacyConfig).Sddl 'legacy rollback changed config ACL'

    $legacyUnowned=New-TestHome 'legacy-unowned';$legacyUnownedConfig=Join-Path $legacyUnowned 'config.toml';Write-Utf8 $legacyUnownedConfig ($basePrefix+'web_search = "cached" # legacy-unowned'+"`n"+$baseSuffix);$legacyUnownedHash=(Get-FileHash $legacyUnownedConfig).Hash;Assert-Equal 0 (Invoke-Tool (Apply-Args $legacyUnowned)).ExitCode 'legacy unowned setup apply';$legacyUnownedState=Read-StateJson $legacyUnowned;$legacyUnownedState.patch_version='0.1.1';$legacyUnownedState.config.web_search_modified=$false;$legacyUnownedState.config.previous_web_search_present=$true;$legacyUnownedState.config.previous_web_search='cached';$legacyUnownedState.config.previous_web_search_literal='"cached"';Write-Json (Join-Path $legacyUnowned 'provider-compat-state.json') $legacyUnownedState;Assert-Equal 0 (Invoke-Tool (Status-Args $legacyUnowned)).ExitCode 'legacy unowned state status';Assert-Equal 0 (Invoke-Tool (Rollback-Args $legacyUnowned)).ExitCode 'legacy unowned state rollback';Assert-Equal $legacyUnownedHash (Get-FileHash $legacyUnownedConfig).Hash 'legacy unowned Web Search changed during rollback'

    $legacyAbsent=New-TestHome 'legacy-none';$legacyAbsentConfig=Join-Path $legacyAbsent 'config.toml';Write-Utf8 $legacyAbsentConfig ($basePrefix+$baseSuffix);$legacyAbsentHash=(Get-FileHash $legacyAbsentConfig).Hash;Assert-Equal 0 (Invoke-Tool (Apply-Args $legacyAbsent)).ExitCode 'legacy absent setup apply';$legacyAbsentText='web_search = "live"'+"`n"+[IO.File]::ReadAllText($legacyAbsentConfig,[Text.Encoding]::UTF8);Write-Utf8 $legacyAbsentConfig $legacyAbsentText;$legacyAbsentState=Read-StateJson $legacyAbsent;$legacyAbsentState.patch_version='0.1.1';$legacyAbsentState.config.web_search_modified=$true;$legacyAbsentState.config.previous_web_search_present=$false;$legacyAbsentState.config.previous_web_search=$null;$legacyAbsentState.config.previous_web_search_literal=$null;Write-Json (Join-Path $legacyAbsent 'provider-compat-state.json') $legacyAbsentState
    Assert-Equal 0 (Invoke-Tool (Status-Args $legacyAbsent)).ExitCode 'legacy absent status';Assert-Equal 0 (Invoke-Tool (Rollback-Args $legacyAbsent)).ExitCode 'legacy absent rollback';Assert-Equal $legacyAbsentHash (Get-FileHash $legacyAbsentConfig).Hash 'legacy rollback did not remove tool-owned web_search'

    $legacyDrift=New-TestHome 'legacy-drift';$legacyDriftConfig=Join-Path $legacyDrift 'config.toml';Write-Utf8 $legacyDriftConfig ($basePrefix+'web_search = "disabled"'+"`n"+$baseSuffix);Assert-Equal 0 (Invoke-Tool (Apply-Args $legacyDrift)).ExitCode 'legacy drift setup apply';$legacyDriftText=[IO.File]::ReadAllText($legacyDriftConfig,[Text.Encoding]::UTF8).Replace('web_search = "disabled"','web_search = "cached"');Write-Utf8 $legacyDriftConfig $legacyDriftText;$legacyDriftState=Read-StateJson $legacyDrift;$legacyDriftState.patch_version='0.1.1';$legacyDriftState.config.web_search_modified=$true;$legacyDriftState.config.previous_web_search_present=$true;$legacyDriftState.config.previous_web_search='disabled';$legacyDriftState.config.previous_web_search_literal='"disabled"';Write-Json (Join-Path $legacyDrift 'provider-compat-state.json') $legacyDriftState;$driftBefore=Snapshot-TestRoot $legacyDrift
    Assert-Equal 3 (Invoke-Tool (Status-Args $legacyDrift)).ExitCode 'legacy web_search drift status';Assert-Equal 3 (Invoke-Tool (Rollback-Args $legacyDrift)).ExitCode 'legacy web_search drift rollback';Assert-Equal $driftBefore (Snapshot-TestRoot $legacyDrift) 'legacy drift handling wrote files'
...
'@
    ...
    $apply=Invoke-Tool (Apply-Args $CodexRoot);Assert-Equal 0 $apply.ExitCode $apply.Output;$text=[IO.File]::ReadAllText($config,[Text.Encoding]::UTF8);Assert-Contains $text 'model_catalog_json = "inside multiline"' 'multiline content changed';Assert-Contains $text '"model_catalog_json" = "' 'quoted catalog key style lost';Assert-Contains $text "'web_search' = `"disabled`" # keep-web-comment" 'quoted web key/comment changed';$stateText=[IO.File]::ReadAllText((Join-Path $CodexRoot 'provider-compat-state.json'),[Text.Encoding]::UTF8);Assert-NotContains $stateText 'keep-web-comment' 'state leaked comment';$state=$stateText|ConvertFrom-Json;Assert-Equal $false $state.config.web_search_modified 'quoted web_search became tool-owned'
...
Test-Case 'hard exit before initial journal rename is recovered from the stale lock nonce' {
    ...
    Remove-Item -LiteralPath $decoy -Force;Assert-NoAtomicTemps $CodexRoot 'initial journal recovery left an atomic temp';$healthy=Invoke-Tool (Status-Args $CodexRoot);Assert-Equal 0 $healthy.ExitCode $healthy.Output;$cleanupRollback=Invoke-Tool (Rollback-Args $CodexRoot);Assert-Equal 0 $cleanupRollback.ExitCode $cleanupRollback.Output;Assert-NoAtomicTemps $CodexRoot 'initial journal cleanup rollback left an atomic temp'
}
...
Test-Case 'version discovery and doctor conclusions are explicit and conservative' {
    ...
    $webIgnored=New-TestHome 'doctor-web';Write-Utf8 (Join-Path $webIgnored 'config.toml') "web_search = [`n  `"live`",`n  `"cached`",`n]`n'web_search' = `"`"`"disabled`"`"`"`nweb_search.mode = `"live`"`nmodel = `"gpt-5.6-sol`"`nmodel_provider = `"custom`"`n";$webDoctor=Invoke-Tool @('doctor','--codex-home',$webIgnored,'--catalog-file',$catalog) @{CODEX_PROVIDER_COMPAT_TEST_VERSIONS='cli=0.144.1'};Assert-Equal 0 $webDoctor.ExitCode $webDoctor.Output;Assert-NotContains $webDoctor.Output 'web_search' 'doctor printed or evaluated the user Web Search setting'
    ...
    $conflictBefore=Snapshot-TestRoot $custom
    $conflict=Invoke-Tool @('apply','--yes','--codex-home',$custom,'--catalog-file',$catalog) @{CODEX_PROVIDER_COMPAT_TEST_VERSIONS='PATH CLI=0.144.3;Desktop runtime=0.144.2;Codex home app-server=0.144.2'}
    Assert-Equal 3 $conflict.ExitCode 'version conflict'
    Assert-Contains $conflict.Output 'version source: PATH CLI -> 0.144.3' 'CLI conflict source missing'
    Assert-Contains $conflict.Output 'version source: Desktop runtime -> 0.144.2' 'Desktop conflict source missing'
    Assert-Contains $conflict.Output 'version source: Codex home app-server -> 0.144.2' 'app-server conflict source missing'
    Assert-Contains $conflict.Output 'Codex surfaces can update' 'version conflict explanation missing'
    Assert-Contains $conflict.Output 'share one Codex home and model catalog' 'version conflict shared-catalog explanation missing'
    Assert-Contains $conflict.Output 'use --codex-version only for the CLI or Desktop surface' 'version conflict action missing'
    Assert-Contains $conflict.Output 'fully' 'version conflict restart emphasis missing'
    Assert-Contains $conflict.Output 'restart' 'version conflict restart guidance missing'
    Assert-Equal $conflictBefore (Snapshot-TestRoot $custom) 'version conflict changed Codex home'
    $sameVersion=Invoke-Tool @('apply','--dry-run','--yes','--codex-home',$custom,'--catalog-file',$catalog) @{CODEX_PROVIDER_COMPAT_TEST_VERSIONS='PATH CLI=0.144.3;Desktop runtime=0.144.3;Codex home app-server=0.144.3'}
    Assert-Equal 0 $sameVersion.ExitCode 'same version sources were not deduplicated'
    $explicit=Invoke-Tool @('apply','--dry-run','--yes','--codex-home',$custom,'--codex-version','0.144.1','--catalog-file',$catalog) @{CODEX_PROVIDER_COMPAT_TEST_VERSIONS='cli=bad;desktop=0.143.0'};Assert-Equal 0 $explicit.ExitCode 'explicit version';$none=Invoke-Tool @('apply','--yes','--codex-home',$custom,'--catalog-file',$catalog) @{CODEX_PROVIDER_COMPAT_TEST_VERSIONS='cli='};Assert-Equal 3 $none.ExitCode 'undetected version'
}
...
Test-Case 'removed web search option exits before path resolution, discovery, network, or writes' {
    $fallbackRoot=New-TestHome 'removed-web';$configPath=Join-Path $fallbackRoot 'config.toml';Copy-Fixture 'config-basic.toml' $configPath;$before=Snapshot-TestRoot $fallbackRoot
    $result=Invoke-ToolWithExactArguments @('apply','--enable-web-search','--yes','--codex-home','','--codex-version','','--catalog-file','') @{CODEX_HOME=$fallbackRoot;CODEX_PROVIDER_COMPAT_TEST_VERSIONS='cli=';CODEX_PROVIDER_COMPAT_TEST_DOWNLOAD_MODE='404'}
    Assert-Equal 1 $result.ExitCode $result.Output;Assert-NotContains $result.Output '--codex-home must not be empty' 'path validation ran before the removed-option guard';Assert-Equal $before (Snapshot-TestRoot $fallbackRoot) 'removed option reached a write path'
    $preAuth=Invoke-Tool @('apply','--enable-web-search','--codex-home') @{CODEX_PROVIDER_COMPAT_TEST_FAIL_STAGE='apply-prepared'} $false;Assert-Equal 1 $preAuth.ExitCode $preAuth.Output;Assert-Contains $preAuth.Output 'was removed' 'raw removed-option guard did not run';Assert-NotContains $preAuth.Output 'internal test hooks are disabled' 'internal test authorization ran before the removed-option guard';Assert-NotContains $preAuth.Output 'requires a value' 'argument parsing ran before the removed-option guard';Assert-Equal $before (Snapshot-TestRoot $fallbackRoot) 'raw removed-option guard reached a write path'
    $mixedCase=Invoke-Tool @('doctor','--ENABLE-WEB-SEARCH','--unknown-later') @{CODEX_PROVIDER_COMPAT_TEST_FAIL_STAGE='apply-prepared'} $false;Assert-Equal 1 $mixedCase.ExitCode $mixedCase.Output;Assert-Contains $mixedCase.Output 'was removed' 'case-insensitive removed-option guard did not run';Assert-NotContains $mixedCase.Output 'unknown argument' 'argument parsing ran before the case-insensitive removed-option guard'
    foreach($ownedName in @('provider-compat-state.json','provider-compat-transaction.json','provider-compat.lock','model-catalogs')){Assert-False (Test-Path -LiteralPath (Join-Path $fallbackRoot $ownedName)) "removed option created $ownedName"}
}
